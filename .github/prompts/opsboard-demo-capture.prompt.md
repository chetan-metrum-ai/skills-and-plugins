# opsboard-demo-capture

Capture Git-backed test evidence, screenshots, and demo references for an OPSBOARD sprint. Use when a task or sprint is ready for review, needs visible acceptance evidence, or must update the read-only dashboard demo view.

This prompt delegates to the agent skill at `../skills/opsboard-demo-capture/`.

## Skill Location

- **Path:** `../skills/opsboard-demo-capture/`
- **Manifest:** `../skills/opsboard-demo-capture/SKILL.md`

## Usage

Invoke this skill by typing `/opsboard-demo-capture` in the Copilot chat.
